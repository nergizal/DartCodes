...
7)Yukarı durum yüzünden sınıfımız içerisinde set metodunu oluşturuyoruz:
 ...
 CemberDaire(int yariCap) {
  ...
  Bunu yaparak artık kullanıcı direkt  _yariCap a ulaşamıyor bana yariCap diye bir değer gönderiyor(satır 79) ve ben bunu set metotuna gönderiyor
...
//User Sınıfı (Base / Superclass)
//Tüm kullanıcı türlerinin ortak özelliklerini içeriyor (email, password, giriş metodu).
//girisYap() metodu diğer sınıflarda override ediliyor.
class User {
  String email = "";
  String password = "";
  void girisYap() {
...
//User'dan kalıtım alır.
//davetEt() gibi sadece Normal kullanıcıya ait fonksiyon içerir.
//girisYap() override edilmiştir.
class NormalUser extends User {
  void davetEt() {
...
//NormalUser'dan kalıtım alır, bu sayede User'ın da özelliklerine ulaşır.
//Sadece okuma yetkisi vardır.
//Giriş metodu kendine özel override edilmiştir.
...
//User'dan kalıtım alır.
//Sadece admin’e özel işlemler içerir (toplam kullanıcı sayısı)
//girisYap() özelleştirilmiş.
class AdminUser extends User {
  void toplamKullaniciSayisiniGoster() {
    print("Toplam kullanıcı sayısı 20");
  }
...
//Her sınıf kendi girisYap() metodunu kendine özel şekilde tanımlar.
//Ana sınıftaki metodu ezerek davranışı değiştirir.
Main :
...
/* Polymorphism, nesne yönelimli programlamada aynı isimdeki bir metodun farklı sınıflarda farklı davranışlar göstermesi anlamına gelir
Aynı isimde bir metod var (girisYap()),
Ama her sınıf bu metodu kendine özel şekilde kullanıyor.
Temel sınıf: User
Ama NormalUser, AdminUser, SadeceOkuyabilenNormalUser sınıfları 
bu metodu kendi ihtiyaçlarına göre değiştiriyor (override ediyor):
Yani User tipiyle işlem yapıyorsun ama her kullanıcı türü kendi davranışını gösteriyor.
İşte buna çok biçimlilik (polymorphism) denir.
